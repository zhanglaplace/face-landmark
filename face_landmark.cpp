...
int main()
{
    ...
    string weights = "./model/VanFace.caffemodel";
	string root_str = "./images/result_";
    ...
    while(getline(fin,filename)){
		...
		int n = filename.find("images");
		if( n != string::npos){
			savename = root_str+filename.substr(n+7);
            cout<<savename<<endl;
        }
		else{
			cout<<"readImg error."<<filename<<endl;
			continue;
		}
        if(image.empty()){
            cout<<"readImg error."<<filename<<endl;
